...
the information that a Krause file might contin is included here (see ICU::Tournament::Krause for more details).
...
Then the results for each round are added using the ID numbers to refer the players.
...
Then finally, to create the file:
  ...
  open('match.txt', 'w') { |f| f.puts @t.serialze('Krause') }
...
from the parser (_parser.error_).
